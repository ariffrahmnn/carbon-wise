...
class AdminRepository {
  ...
  async getAllStudentRecords({ search = '', school = '', classGrade = '', page = 1, limit = 10 }) {
    ...
    const mainQuery = `
      ...
      LEFT JOIN (
        ...
        WHERE DATE((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta') = (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Jakarta')::DATE
        GROUP BY user_id
      ) daily ON u.id = daily.user_id
      LEFT JOIN (
        SELECT user_id, SUM(total_batch_co2) AS total_weekly
        FROM calculation_batches
        WHERE ((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta')::DATE >= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Jakarta')::DATE - INTERVAL '6 days'
        GROUP BY user_id
      ) weekly ON u.id = weekly.user_id
      LEFT JOIN (
        SELECT user_id, SUM(total_batch_co2) AS total_monthly
        FROM calculation_batches
        WHERE EXTRACT(MONTH FROM ((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta')) = EXTRACT(MONTH FROM (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Jakarta'))
          AND EXTRACT(YEAR FROM ((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta')) = EXTRACT(YEAR FROM (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Jakarta'))
        GROUP BY user_id
  ...
  async getStudentAnalytics(userId, targetMonth, targetYear) {
    ...
    const dailyQuery = `
      SELECT 
        TO_CHAR((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta', 'HH24:MI') AS formatted_time,
        TRIM(TO_CHAR((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta', 'Day')) AS day_name,
        TO_CHAR((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta', 'DD Mon YYYY') AS formatted_date,
        SUM(total_batch_co2) AS total 
      FROM calculation_batches 
      WHERE user_id = $1 AND DATE((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta') = (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Jakarta')::DATE 
    ...
    const weeklyQuery = `
      SELECT 
        ...
        SUM(total_batch_co2) AS total 
      FROM calculation_batches 
      WHERE user_id = $1 AND ((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta')::DATE >= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Jakarta')::DATE - INTERVAL '6 days' 
    ...
    const monthlyQuery = `
      SELECT 
        'Week ' || CEIL(EXTRACT(DAY FROM ((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta')) / 7.0) AS week, 
        SUM(total_batch_co2) AS total 
      FROM calculation_batches 
      WHERE user_id = $1 
        AND EXTRACT(MONTH FROM ((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta')) = $2 
        AND EXTRACT(YEAR FROM ((created_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta')) = $3 
      GROUP BY week 
      ORDER BY week ASC
    `;
